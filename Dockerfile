...
LABEL Description="QGis3 Desktop" Version="1.1" maintainer="krzysztof@krawczyk.czest.pl"
...
RUN groupadd -g $USER_GROUP_ID $USER_GROUP_NAME \
    && useradd -m --shell /bin/bash --uid $USER_ID --gid $USER_GROUP_ID $USER_NAME \
    && apt update
...
RUN  apt install -y gpg wget \
    ...
    && apt update \
    && DEBIAN_FRONTEND=noninteractive TZ=UTC apt install -y graphviz tzdata qgis grass \
    && apt clean \
